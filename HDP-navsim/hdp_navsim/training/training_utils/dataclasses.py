...
class Scene:
    ...
    def get_future_trajectory_at_specific_frame(self, frame_id, num_trajectory_frames: Optional[int] = 8) -> Trajectory:
        """
        Extracts future trajectory of the human operator in local coordinates (ie. ego rear-axle).
        :param num_trajectory_frames: optional number frames to extract poses, defaults to None
        :return: trajectory dataclass
    ...
    def get_history_trajectory_at_specific_frame(self, frame_id, num_trajectory_frames: Optional[int] = None) -> Trajectory:
        """
        Extracts past trajectory of ego vehicles in local coordinates (ie. ego rear-axle).
        :param num_trajectory_frames: optional number frames to extract poses, defaults to None
        :return: trajectory dataclass
        ...
        global_ego_poses = []
        for frame_idx in range(frame_id + 1 - num_trajectory_frames, frame_id + 1):
            global_ego_poses.append(self.frames[frame_idx].ego_status.ego_pose)
    ...
    def get_agent_input_at_specific_frame(self, frame_id) -> AgentInput:
        """
        Extracts agents input dataclass (without privileged information) from scene.
        :return: agent input dataclass
        """

        local_ego_poses = self.get_history_trajectory_at_specific_frame(frame_id).poses

        ego_statuses: List[EgoStatus] = []
        cameras: List[Cameras] = []
        lidars: List[Lidar] = []

        for idx in range(self.scene_metadata.num_history_frames):

            frame_idx = frame_id + 1 - self.scene_metadata.num_history_frames + idx
            
            frame_ego_status = self.frames[frame_idx].ego_status

            ego_statuses.append(
                EgoStatus(
                    ego_pose=local_ego_poses[idx],
                    ego_velocity=frame_ego_status.ego_velocity,
                    ego_acceleration=frame_ego_status.ego_acceleration,
                    driving_command=frame_ego_status.driving_command,
                )
...
class SceneFilter:
    ...
    log_names_tokens_path: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        if self.log_names_tokens_path is not None:
            with open(self.log_names_tokens_path, "r", encoding="utf-8") as file:
                log_names_tokens = json.load(file)
            
            self.log_names = log_names_tokens['log_names']
            self.tokens = log_names_tokens['tokens']
