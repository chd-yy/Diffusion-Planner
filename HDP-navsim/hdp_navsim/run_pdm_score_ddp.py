...
from hdp_navsim.training.training_utils.dataset import CacheOnlyDataset, Dataset
...
from hdp_navsim.paths import metric_cache_path, navsim_devkit_root
...
CONFIG_PATH = str(
    navsim_devkit_root() / "navsim" / "planning" / "script" / "config" / "pdm_scoring"
)
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    train_data = Dataset(
        ...
        feature_builders=agent.get_feature_builders(True),
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            features, _, _ = train_data.load_token_cache(str(train_data._valid_cache_paths[token].resolve()))
            features = {k:v.to(device).unsqueeze(0) for k, v in features.items()}
            ...
            logger.info(f"[Scene {token}] score: {pdm_result.score}")
        ...
        pdm_scores.append(pdm_result.score)
        logger.info(f"Intermidiate average score: {np.mean(pdm_scores)}")
